...
class TestFlaskVirtualHome(unittest.TestCase):
    ...
    def test_002_created_devices(self):
        ...
        self.assertTrue(device_types ==
                        set(['wifi_ap', 'GenericNode', 'Generic Node']),
    ...
    def test_003_connect_to_thread_network(self):
        home_id = list(
            requests.get(
                urljoin(SERVICE_URL, 'get_homes')).json())[0]
        home_devices = requests.get(
            urljoin(SERVICE_URL, 'home_devices/{}'.format(
                home_id))).json().values()
        device_ids = [
            device['id'] for device in home_devices
    ...
    def test_006_device_connectivity(self):
        ...
        ret = requests.get(
            urljoin(SERVICE_URL, 'device_cmd/{}/{}/{}?stream={}'.format(
                home_id,
                dev_addrs[0][0],
                ping_command.format(dev_addrs[1][1]),
                False)), stream=False).json()
