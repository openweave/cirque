...
class TestHome(unittest.TestCase):
    ...
    def test_001_create_home(self):
        ...
        generic_node_id = next(node_id for node_id, desc
                               in description.items()
                               if desc['type'] == 'generic_node_image')
        wifi_ap_id = next(node_id for node_id, desc
                          in description.items()
        ...
        self.__verify_process_in_container(
            generic_node_id, 'wpa_supplicant')
        if self.weave_crt_path is not None:
            self.__verify_process_in_container(
                generic_node_id, 'nldaemon')
    ...
    def test_004_docker_network_access(self):
        ...
        self.assertEqual(
            host_run(logger, '{} {}'.format(ping_cmd, device0_ip)).returncode,
            0
        )
        ...
        self.assertEqual(
            host_run(
                logger,
                '{} {}'.format(ping_cmd, internal_node_ip)).returncode,
            0
        )
        self.__verify_ping_in_container(
            internal_node_id, '8.8.8.8',
            ping_cmd=ping_cmd, should_succeed=False)
        home.destroy_home()

    def __verify_ping_in_container(self, node_id, address,
                                   ping_cmd='ping -c 1',
                                   should_succeed=True):
