...
class CirqueHome:
    ...
    def add_device(self, device_config):
        ...
        if 'capability' in device_config:
            for capability_name in device_config['capability']:
                capability = self.__make_capability(
                    capability_name, device_config)
                if capability is not None:
                    capabilities.append(capability)
        if device_type == 'wifi_ap':
            device_node = WiFiAPNode(
                self.docker_client, base_image=base_image)
        else:
            device_node = DockerNode(
                self.docker_client, device_type, capabilities,
    ...
    def __make_capability(self, capability, device_config):
        ...
        if capability not in factory_functions:
            self.logger.critical(
                'Unsupported capability {}'.format(capability))
            return None
        else:
    ...
    def __make_xvnc_capability(self, capability, device_config):
        ...
        return XvncCapability(localhost)
    ...
    def get_home_devices(self):
        return {node.id: self.get_device_state(
            node.id) for node in self.home['devices'].values()}
