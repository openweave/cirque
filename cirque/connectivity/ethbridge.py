...
class EthBridge:
    ...
    def manage_nat_rule(network, action):
        ...
        subprocess.call([iptable_cmd, '-t', 'nat', action_arg,
                         'POSTROUTING', '-s', str(network), '-j', 'MASQUERADE']
                        )
    ...
    def open(self):
        with IPRoute() as ipr:
            ...
            ipr.addr('add', index=self.bridge_link_id, address=str(
                self.ipv4_addr), prefixlen=self.ipv4_network.prefixlen)
            ipr.addr('add', index=self.bridge_link_id, address=str(
                self.ipv6_addr), prefixlen=self.ipv6_network.prefixlen)
