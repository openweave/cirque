...
class IpNameSpace:
    def __init__(self, bridge):
        ...
        self.host_interface_name = 'veth{}{}'.format(
            bridge.bridge_id, self.ns_id)
        self.ns_interface_name = 'vetp{}{}'.format(
            bridge.bridge_id, self.ns_id)
        self.ipv4_addr, self.ipv6_addr = map(
            str, bridge.get_new_device_addr())

    def __setup_veth_pair(self):
        with IPRoute() as ipr:
            ipr.link('add', ifname=self.host_interface_name,
                     peer=self.ns_interface_name, kind='veth')
            self.host_interface_id = ipr.link_lookup(
                ifname=self.host_interface_name)[0]
            ns_interface_id = ipr.link_lookup(
                ifname=self.ns_interface_name)[0]
            ipr.link('set', index=ns_interface_id,
                     net_ns_fd=self.namespace_name)
            ipr.link('set', index=self.host_interface_id,
                     master=self.bridge.bridge_link_id)
            ipr.link('set', index=self.host_interface_id, state='up')

    def __setup_ip_namespace_route(self):
        ns_interface_id = self.netns.link_lookup(
            ifname=self.ns_interface_name)[0]
        self.netns.link('set', index=ns_interface_id, state='up')
        self.netns.addr('add', index=ns_interface_id,
                        address=self.ipv4_addr,
                        prefixlen=self.bridge.ipv4_prefix_len)
        self.netns.addr('add', index=ns_interface_id,
                        address=self.ipv6_addr,
                        prefixlen=self.bridge.ipv6_prefix_len)
        self.netns.route('add', dst='0.0.0.0/0',
                         gateway=str(self.bridge.ipv4_addr))
        self.netns.route(
            'add',
            dst='::/0',
            gateway=str(
                self.bridge.ipv6_addr))
