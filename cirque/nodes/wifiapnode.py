...
class WiFiAPNode(DockerNode):
    def __init__(self, docker_client, ssid=None,
                 password=None, container_name=None,
    ...
    def __run_hostapd(self):
        ...
        if ret.exit_code != 0:
            raise ContainerExecError(
                "unable to start up hostapd server!!")
    ...
    def __run_dnsmasq(self):
        ...
        if ret.exit_code != 0:
            raise ContainerExecError(
                "unable to start up dnsmasq server!!")
    ...
    def __setup_network_forward_rules(self):
        ...
        for rule in forward_rules:
            ...
            if ret.exit_code != 0:
                raise ContainerExecError(
                    "unable to set forward rule: {}".format(rule))
    ...
    def __update_hostapd_password(self):
        ...
        if ret.exit_code != 0:
            raise ContainerExecError(
                "failed to set wifi ap password: {}".format(self.pasword))
