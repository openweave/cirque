...
class TestTrafficControl(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.node1 = DockerNode(docker_client, 'generic_node_image',
    ...
    def test_001_ping(self):
        ret = self.node1.container.exec_run(
            "ping -q -c 25 {}".format(self.node2.description['ipv4_addr']))
        print("Ping command output: {}".format(ret.output.decode()))
        self.assertTrue(ret.exit_code == 0)
        received = float(re.findall(rb'\s+(\d+)\sreceived',
