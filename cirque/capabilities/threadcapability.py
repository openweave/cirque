...
class ThreadCapability(BaseCapability):
    def __init__(self, node_id, petition_id, daemons=[
                 'wpantund', 'otbr-agent'], rcp=False):
    ...
    def get_docker_run_args(self, dockernode):
        return {
            'devices': ['{}:/dev/ttyUSB0'.format(
                self.thread_endpoint.pipe_path_for_user)],
            'volumes': ['{}:/dev/ttyUSB0'.format(
                self.thread_endpoint.pipe_path_for_user)],
            'sysctls': {
